# Provides simple HTTP server. Generates HTTP reply content in two packets, second packet contains data and FIN useful for testing WFP Stream filter special scenario related to race condition in WFP engine
# Source based on # Akaljed code, http://www.akaljed.wordpress.com
...
print "Waiting for GET request on IP: %s port: %s " %(local_ip, listening_port)
